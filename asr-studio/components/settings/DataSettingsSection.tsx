...
export const DataSettingsSection: React.FC<DataSettingsSectionProps> = ({
  ...
  return (
    <div className="space-y-5">
      <SectionBlock title="系统工具" icon={<ToolboxIcon className="h-4 w-4" />}>
        <SettingRow
          label="安装应用"
      ...
      <SectionBlock
        title="缓存"
        icon={<StorageIcon className="h-4 w-4" />}
        description="清理后不影响历史记录，仅丢弃可重建的临时数据。"
      >
        <SettingRow
          label="清除识别缓存"
          description="删除同一音频重复识别时复用的结果缓存。"
          icon={<StorageIcon className="h-4 w-4" />}
        >
          <button
            type="button"
      ...
      <section className="rounded-lg border border-red-500/25 bg-red-500/10 p-4 text-red-700 shadow-sm sm:p-5 dark:text-red-300">
        <div className="mb-3 flex items-start gap-2">
          <WarningIcon className="mt-0.5 h-4 w-4 flex-shrink-0" />
          <div>
            <h4 className="text-sm font-semibold">危险操作</h4>
            <p className="mt-0.5 text-xs leading-relaxed opacity-80">以下操作不可撤销，请谨慎确认。</p>
          </div>
        </div>
